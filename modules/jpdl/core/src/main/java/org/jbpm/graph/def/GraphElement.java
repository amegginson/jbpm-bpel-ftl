...
public abstract class GraphElement implements Serializable
{
  ...
  /**
   * indicative set of event types supported by this graph element. this is currently only used by the process designer to know which event types to show on a given
   * graph element. in process definitions and at runtime, there are no contstraints on the event-types.
   */
  ...
  /*
   * // the next instruction merges the actions specified in the process definition with the runtime actions List actions = event.collectActions(executionContext); //
   * loop over all actions of this event Iterator iter = actions.iterator(); while (iter.hasNext()) { Action action = (Action) iter.next();
   * executionContext.setAction(action); if ( (!isPropagated) || (action.acceptsPropagatedEvents() ) ) { // create action log ActionLog actionLog = new
   * ActionLog(action); executionContext.getToken().startCompositeLog(actionLog); try { // execute the action action.execute(executionContext); } catch (Exception
   * exception) { // NOTE that Error's are not caught because that might halt the JVM and mask the original Error.
   * Event.log.error("action threw exception: "+exception.getMessage(), exception); // log the action exception actionLog.setException(exception); // if an exception
   * handler is available event.graphElement.raiseException(exception, executionContext); } finally { executionContext.getToken().endCompositeLog(); } } } }
   */
   ...
   * hierarchy. If an exception handler is found, it is applied. If the exception handler does not throw an exception, the exception is considered handled. Otherwise
   * the search for an applicable exception handler continues where it left of with the newly thrown exception.
   */
