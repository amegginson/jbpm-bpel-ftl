...
#
echo ""
echo "Running `basename $0` $LastChangedDate: 2009-11-26 12:07:43 -0500 (Thu, 26 Nov 2009) $"
echo ""
echo "Started : `date`"
echo "Params  : $@"
echo ""
...
case ${BUILD_SYS} in
    ...
    BUILD_SYS="macosx"
    so_extension=dylib
    # Try to use darwinports instaled in /opt/local
    # we need wget
...
checkdownload()
{
  ...
  if [ ! -d ${src_dir} ]; then
    ...
    if [ $? -ne 0 ]; then
      echo "Can't download $src_url $src_url_back"
      exit 1
    fi
    $build_top/util/overridecp.sh $build_top/../../srclib/ ${name_prod} ${version_prod} NMAKEmakefile $src_dir
...
if $BUILD_WIN ; then
  ...
  APRURLBACK=http://archive.apache.org/dist/apr/apr-${apr_version}-win32-src.zip
...
# apr-util depends on apr-iconv for xlate.c (1.3.x).
if $has_apu; then
  checkdownload apr-util ${apu_version} $APUURL $APUURLBACK
  echo "Copying iconv include from $package_src_dir/srclib/apr-iconv-${api_version}/include"
  if [ -f $package_src_dir/srclib/apr-iconv-${api_version}/include/apr_iconv.h ]; then
    cp $package_src_dir/srclib/apr-iconv-${api_version}/include/* $package_src_dir/srclib/apr-util-${apu_version}/include
  fi
fi
if $has_expat; then
...
echo ""
echo "SUCCESS : `basename $0` $LastChangedDate: 2009-11-26 12:07:43 -0500 (Thu, 26 Nov 2009) $"
echo ""
